#' selectInput_GET_compartment_replicates UI Function
#'
#' @description A shiny::selectInput wrapper around the deepomics' API GET compartment_replicates call.
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_selectInput_GET_compartment_replicates_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' selectInput_GET_compartment_replicates Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package
#' @param api_function_options List of options to pass to GET /compartment_replicates as query parameters
...
#'  \item{options}{The options used to call GET /compartment_replicates
...
#'  \item{result_df}{A dataframe containing the results of GET /compartment_replicates call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_selectInput_GET_compartment_replicates_server <- function(id,
  ...
  moduleServer( id, function(input, output, session){
    ...
    output$choix <- shiny::renderUI({
      ...
      label <- 'Choose an compartment replicate:'
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more compartment replicate(s):'
      }
