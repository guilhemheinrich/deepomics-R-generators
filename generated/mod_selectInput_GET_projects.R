#' selectInput_GET_projects UI Function
#'
#' @description A shiny::selectInput wrapper around the deepomics' API GET projects call.
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_selectInput_GET_projects_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' selectInput_GET_projects Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package
#' @param api_function_options List of options to pass to GET /projects as query parameters
...
#'  \item{options}{The options used to call GET /projects
...
#'  \item{result_df}{A dataframe containing the results of GET /projects call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_selectInput_GET_projects_server <- function(id,
  ...
  moduleServer( id, function(input, output, session){
    ...
    output$choix <- shiny::renderUI({
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more Project(s):'
      }
