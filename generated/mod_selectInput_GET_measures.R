#' selectInput_GET_measures UI Function
#'
#' @description A shiny::selectInput wrapper around the deepomics' API GET measures call.
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_selectInput_GET_measures_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' selectInput_GET_measures Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package
#' @param api_function_options List of options to pass to GET /measures as query parameters
...
#'  \item{options}{The options used to call GET /measures
...
#'  \item{result_df}{A dataframe containing the results of GET /measures call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_selectInput_GET_measures_server <- function(id,
  ...
  moduleServer( id, function(input, output, session){
    ...
    output$choix <- shiny::renderUI({
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more measure(s):'
      }
