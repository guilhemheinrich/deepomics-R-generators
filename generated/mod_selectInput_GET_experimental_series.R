#' selectInput_GET_experimental_series UI Function
#'
#' @description A shiny::selectInput wrapper around the deepomics' API GET experimental_series call.
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_selectInput_GET_experimental_series_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' selectInput_GET_experimental_series Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package
#' @param api_function_options List of options to pass to GET /experimental_series as query parameters
...
#'  \item{options}{The options used to call GET /experimental_series
...
#'  \item{result_df}{A dataframe containing the results of GET /experimental_series call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_selectInput_GET_experimental_series_server <- function(id,
  ...
  moduleServer( id, function(input, output, session){
    ...
    output$choix <- shiny::renderUI({
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more Experimental Serie(s):'
      }
